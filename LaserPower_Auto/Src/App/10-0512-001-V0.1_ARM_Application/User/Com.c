...
/*
    ...
    01c, 04Dec23, Karl Implemented iCmdQueryInfo, rCmdStatusInfo andrCmdDiagInfo 
    01d, 27Dec23, Karl Modified prvSendStatusInfo to force currentadc as 0 when it is greater than 3200 
    ...
    01k, 20Jan24, Karl Added usSysStatus andulPwrStatus in RCmdStatusInfo_t 
...
#define SET_COM_SEND_PTL        1 /* 1: Jasper; 2: Karl; */

/* Local types */
#pragma pack(push)
#pragma pack(1)
enum { CHAN_COM, CHAN_NET };
...
static void prvCmdParaConfig(uint8_t ucSrcAddr, const uint8_t *pucCont, uint32_t ulLength, void *pvInfo) {
    ...
    case TYPE_MODULE_ENABLE:
        ...
        case 0:
            th_ModEn.MANUAL = pxData->ulPara2 ? 1 : 0;
            break;
        case 1:
            th_ModEn.QBH = pxData->ulPara2 ? 1 : 0;
            break;
        case 2:
            th_ModEn.PD = pxData->ulPara2 ? 1 : 0;
            break;
        case 3:
            th_ModEn.TEMP1 = pxData->ulPara2 ? 1 : 0;
            break;
        case 4:
            th_ModEn.TEMP2 = pxData->ulPara2 ? 1 : 0;
            break;
        case 5:
            th_ModEn.CHAN1_CUR = pxData->ulPara2 ? 1 : 0;
            break;
        case 6:
            th_ModEn.CHAN2_CUR = pxData->ulPara2 ? 1 : 0;
            break;
        case 7:
            th_ModEn.CHAN3_CUR = pxData->ulPara2 ? 1 : 0;
            break;
        case 8:
            th_ModEn.WATER_PRESS = pxData->ulPara2 ? 1 : 0;
            break;
        case 9:
            th_ModEn.WATER_CHILLER = pxData->ulPara2 ? 1 : 0;
            break;
        case 10:
            th_ModEn.CHAN_CTRL = pxData->ulPara2 ? 1 : 0;
            break;
        }
        DataSaveDirect();
        break;
...
void prvHexstrToDecstr(const ICmdEncrypt_t *pxData) {
    ...
    // Convert each pair from hexadecimal to decimal and store each digit
    // separately
    for (int i = 0; i < NUM_PAIRS; i++) {
...
static void prvCmdEncrypt(uint8_t ucSrcAddr, const uint8_t *pucCont, uint32_t ulLength, void *pvInfo) {
    ...
    if (((decimalArray[6] - '0') * 1000 + (decimalArray[7] - '0') * 100 + (decimalArray[8] - '0') * 10 +
         (decimalArray[9] - '0')) != th_OrderNum) {
...
static void prvSendStatusInfo(void *pvInfo) {
    ...
    pxData->usAdc[0]      = AdcGet(ADC_CHAN_1); /* ����Դ1���� */
    pxData->usAdc[1]      = AdcGet(ADC_CHAN_2); /* ����Դ1��ѹ */
    pxData->usAdc[2]      = AdcGet(ADC_CHAN_3); /* ����Դ2���� */
    pxData->usAdc[3]      = AdcGet(ADC_CHAN_4); /* ����Դ2��ѹ */
    pxData->usAdc[4]      = AdcGet(ADC_CHAN_5); /* ����Դ3���� */
    pxData->usAdc[5]      = AdcGet(ADC_CHAN_6); /* ����Դ3��ѹ */
