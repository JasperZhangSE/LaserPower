...
static void prvCliCmdStcDiag(cli_printf cliprintf, int argc, char** argv)
{
    ...
    if ((i >= 1) && (i <= DEV_NUM)) {
        ...
        cliprintf("    SwVer   : %d.%d.%d\n", (s_xDiag[i].ulSwVer >> 16) & 0xFF, (s_xDiag[i].ulSwVer >> 8) & 0xFF, (s_xDiag[i].ulSwVer >> 0) & 0xFF);
        ...
        for (uint8_t n = 0; n < DEV_NUM; n++) {
            cliprintf("STC-%d DIAG:\n", n + 1);
            cliprintf("    SwVer   : %d.%d.%d\n", (s_xDiag[n].ulSwVer >> 16) & 0xFF, (s_xDiag[n].ulSwVer >> 8) & 0xFF, (s_xDiag[n].ulSwVer >> 0) & 0xFF);
            cliprintf("    RunTime : %ld\n", s_xDiag[n].ulRunTime);
