 ...
 */
Status_t
CliCustomLoadCmd(const cli_command_t* commands, size_t num_of_commands) {
...
Status_t
CliCustomInit(void) {
    CliCustomLoadCmd(commands, sizeof(commands) / sizeof(commands[0]));
    return STATUS_OK;
}
 ...
 */
void
CliCustomInput(cli_printf cliprintf, char ch) {
    static char last_ch;
